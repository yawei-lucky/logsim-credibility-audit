...
下一步仍按路线 B 审计指标，但先做战略资格审计，不增加接收方或曲线。对保留
的每类证据明确其外部效度来自 matched reality、独立测量、物理/因果规律、
接收方真实数据能力，还是仅有 HUGSIM 内部自指；同时把后续实验明确拆成
matched real-sim direct-equivalence 和无精确现实起点的 designed
counterfactual robustness 两条路径。完成这一步后再选择具体下游 AD 决策边界。
短期缺少真实源数据时保留 source gate，但它只阻断直接 real-sim 等效性主张，
不阻断满足资格与边界要求的人工反事实实验。
