...
- 已增加 fail-closed 配对/时域分析、writer 同值 `Done` 握手、严格
  ...
  matched-pose manifest，并累计 53 个回归测试；
...
- 为 `scene-0041`、`scene-0138` 固定少量原生可见车辆与 nuisance 区域人工
  标注，把无真值 response count 升级为可审计的 precision/error evidence；
...
下一步仍按路线 B 审计指标，但不再增加接收方或曲线。先用合成世界点和逐相机
overlay 核验 HUGSIM vehicle/camera/Sparse4D 坐标与标定变换；HUGSIM actor Z
在相机可投影约定明确前不进入参考。随后对 `scene-0041`、`scene-0138` 固定少量
原生可见车辆及 ego hood、植被、标牌、重建 artifact 区域做人工标注，把正常
场景 baseline 从 response count 升级为 precision/error evidence。完成这两步后，
再判断残余偏差更像接收方 domain shift 还是 simulator/adapter geometry 问题。
短期缺少真实源数据时继续保留 source gate，不用新增 proxy 曲线替代。
