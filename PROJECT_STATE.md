...
当前最小行动进一步收敛为：先建立带人工/独立可见目标核验的正常场景
receiver baseline，区分真实可见车辆与 ego hood、植被、标牌等 nuisance false
positive；在此之前不再把 detector 最大框或 raw center-path risk 当作跨场景风险
量。并以 `scene-0138` 左前/左后视角作为内部投影与遮挡诊断的首个靶点。
