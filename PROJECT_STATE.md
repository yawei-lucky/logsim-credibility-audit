...
- 已增加 fail-closed 配对/时域分析、writer 同值 `Done` 握手、严格
  ...
  matched-pose manifest，并累计 56 个回归测试；
...
下一步仍按路线 B 审计指标，但不再增加接收方或曲线。跨场景汇总与第一轮
box-bias diagnostic 已完成；当前最小行动是对 `scene-0041`、`scene-0138` 固定
少量原生可见车辆及 ego hood、植被、标牌、bus-stop shelter、重建 artifact
区域做人工标注，把正常场景 baseline 从 response count 升级为 precision/error
evidence。随后选择一个下游 AD 决策边界，把误差是否可接受绑定到 lane/risk/
action invariance，而不是使用通用米制阈值。HUGSIM actor Z 在相机可投影约定
明确前不进入参考。短期缺少真实源数据时继续保留 source gate，不用新增 proxy
曲线替代。
