...
> 在本地或云端执行 HUGSIM smoke test。

最小执行路径：

1. clone HUGSIM；
2. 安装 pixi 环境；
3. 下载 sample_data 或必要 public released assets；
4. 更新 `configs/sim/*_base.yaml` 本地路径；
5. 运行 `scripts/check_hugsim_smoke_prereqs.py`；
6. 启动 HUGSIM debug / closed-loop path；
7. 用 `scripts/hugsim_plan_pipe_writer.py` 提供 deterministic plan；
8. 收集 `data.pkl`、`video.mp4`、`infos.pkl`、`eval.json`、`ground.ply`、`scene.ply`；
9. 写第一条 closed-loop credibility audit record。
