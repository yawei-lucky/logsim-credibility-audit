...
下一步仍按路线 B 审计指标，但不再增加接收方或曲线。跨场景汇总、box-bias
diagnostic 和正常场景固定可见性标注均已完成。当前最小行动是先定义一个下游
AD 决策边界，优先采用 lane relation + critical-object risk ordering，把现有
nuisance 和几何误差是否可接受绑定到排序/动作是否改变，而不是使用通用分数或
米制阈值。HUGSIM actor Z 在相机可投影约定明确前不进入参考。短期缺少真实源
数据时继续保留 source gate，不用新增 proxy 曲线替代。
