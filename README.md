...
- fail-closed 配对/时域检查、同值 FIFO 结束握手、claim/diagnostic
  语义校验、AD readiness 与 matched-pose manifest，以及 56 个回归测试。
...
下一次如继续 HUGSIM，不再优先增加接收方。先对 `scene-0041`、`scene-0138`
固定少量原生目标和 nuisance 区域做人工可见性标注，把 response count 升级为
precision/error evidence；同时把接受阈值绑定到下游 AD 决策裕度。真实源日志
锚点仍是后续 matched real-sim 对比的关键 gate。不要继续修改同一切入参数追结果。
