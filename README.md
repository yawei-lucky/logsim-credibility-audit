...
- fail-closed 配对/时域检查、同值 FIFO 结束握手、claim/diagnostic
  语义校验、AD readiness 与 matched-pose manifest，以及 53 个回归测试。
...
下一次如继续 HUGSIM，不再优先增加接收方。先审计 Sparse4Dv3 绝对位置偏差的
来源：核验车辆/相机/接收方坐标与标定投影，并对 `scene-0041`、`scene-0138`
固定少量原生目标和 nuisance 区域做人工可见性标注。真实源日志锚点仍是后续
matched real-sim 对比的关键 gate。不要继续修改同一切入参数追结果。
