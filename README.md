...
研究主框架已调整为三个问题：**预测结构一致性、动作效应一致性、任务边界一致性**。
定义见 [可信验证方法](CREDIBILITY_VALIDATION_METHOD.md)。原来的日志复现、传感器一致性、
任务级一致性、闭环结果可信性，仅保留为**证据来源和实验归档方式**，不再作为顶层主线。
既有实验和裁决保留；三个问题不是串行阶段，也不意味着指标完备或仿真器已经可信。
...
- fail-closed 配对/时域检查、同值 FIFO 结束握手、claim/diagnostic
  语义校验、AD readiness 与 matched-pose manifest，以及 58 个回归测试。
...
- Sparse4Dv3 驾驶域 3D receiver baseline；冻结官方 nuScenes 权重，只输入
  HUGSIM 六相机 RGB 与标定。车辆注入敏感性、near/far 和 same/adjacent
  关系方向 accepted；绝对 actor XY 一致性 down-weighted，中位误差为
  2.56/4.24/3.80 米。
- Sparse4Dv3 跨场景与 task-envelope 汇总；`scene-0041`、`scene-0138` 已加入
  阈值稳定性、类别与跟踪基线，受控 near/far 为 6/6、车道关系为 43/44；
  当前定位为 task-level receiver-consistency candidate，不是 sensor consistency。
- 框偏差诊断；米制中心有偏差，但回投注入车辆区域的中位 2D IoU 为
  0.69/0.74/0.82，且接收方预测车体更大，scale-depth/domain-shift 解释
  down-weighted，gross image-plane projection failure 不像唯一原因。
- 正常场景 Sparse4Dv3 固定可见性标注；14 个 detection-conditioned 输出中
  7 个有渲染 RGB 可见目标支撑、7 个为路缘/植被/相机边界/重建拖影响应。
  该结果证明 raw response count 不足以单独作为指标，但不是 ODD precision/
  recall 或真实—仿真一致性。
- 验证工具资格审计；现有工具中只保留时域/独立二维几何约束和 Sparse4Dv3
  序数关系响应两类有限用途候选。它们可分别检查干预有效性和一个接收方的方向
  响应，但尚不能判断 HUGSIM 是否足以作为 AD 测试域。
- critical-object / risk-order task-boundary 资格审计；已冻结走廊冲突、首次冲突
  先后和净距的偏序构念与定性稳定性规则，但数值 uncertainty envelope 仍无
  独立依据，不能形成现实风险阈值。
- ordinal metamorphic audit 001；五个预注册条件全部完成，四条几何偏序逐帧
  成立，Sparse4Dv3 可用比较零反转；两条关系 13/13 accepted，两条因同一个
  `adjacent_near@6.5s` 缺失为 12/13 down-weighted。它建立设计范围内的正面因果/
  序数证据，不建立现实阈值或 AD 测试域结论。
...
最新的驾驶域 3D receiver baseline 使用冻结 Sparse4Dv3，并明确排除 HUGSIM
semantic/depth：
...
→ frozen Sparse4Dv3 six-camera 3D receiver
→ causal relation direction accepted
→ absolute XY geometry down-weighted
→ cross-scene threshold / persistence baseline
→ task-relative indicator convergence
→ pixel-aligned but metric-biased box diagnostic
→ fixed human-visible target / nuisance annotation audit
→ validation-instrument qualification
```

下一次不增加 HUGSIM 接收方或场景。ordinal metamorphic audit 001 已按发布后的
预注册完整运行并停止。下一步先用官方/原始真实 nuScenes 证据资格化 Sparse4Dv3
的外部检测/跟踪构念、适用范围和误差/稳定性边界；若公开 benchmark 不足以形成
本任务的 per-scenario acceptance boundary，就记录最小真实标注或第二接收方
需求，不从 HUGSIM 误差反推阈值，也不围绕单个缺失帧调参。
