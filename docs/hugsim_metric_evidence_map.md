...
| Control / outcome | acceleration、steer rate、车辆状态、碰撞和任务结果 | SparseDrive 原生轨迹→受审计 FIFO→修正坐标的 HUGSIM iLQR；当前仅冻结轨迹回放 | 规划能否无改写进入控制接口，以及声明动作是否产生一致的内部状态更新 | 单条充分 warm-up 轨迹已无补点/重复地驱动两个 0.25 s 子步，两次独立复位逐值一致 | 新观测上的 AD 再推理、闭环避碰、动作幅度符合现实或真实车辆安全 | 先完成 SparseDrive live writer 的历史预热与 2 Hz 反馈门；再做预注册冲突闭环，并以匹配真实结果或独立行为范围升级 |
