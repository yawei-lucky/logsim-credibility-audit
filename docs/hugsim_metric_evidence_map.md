...
| Planning | 候选轨迹分数与最终 ego 轨迹 | 冻结 SparseDrive-S Stage2；以六相机 `inverse(v2c) × l2c` 恢复共同 LiDAR→车体标定，并把轨迹投影到原始前视图；VAD-Tiny 仅作后续架构复核 | 冻结 AD 的原生开环规划输出、投影方向及 0.5 s 步长运动学诊断 | 支持运行可达、有限值、状态重置、内部坐标/投影和完整 warm-up 后的单场景基本速度连续性；尚无反事实规划方向或现实等效证据 | 显式关键目标身份、校准风险概率、制动/转向控制、规划一致性或 HUGSIM 可信结论 | 审计 10-D ego status 合理替代并固定等时序 warm-up；稳定后再做预注册反事实方向实验，之后补匹配真实数据或第二架构证据 |
