...
>
> 本文不是最终可信性指标，也不对 HUGSIM 进行逐层打分。顶层研究框架为预测结构一致性、动作效应一致性、任务边界一致性，定义见 [可信验证方法](../CREDIBILITY_VALIDATION_METHOD.md)。原四层仅保留为证据来源和实验归档方式，不再作为顶层主线；下文既有指标、实验和裁决不因此改写。
...
| Planning | 候选轨迹分数与最终 ego 轨迹 | 冻结 SparseDrive-S Stage2；以六相机 `inverse(v2c) × l2c` 恢复共同 LiDAR→车体标定，并把轨迹投影到原始前视图 | 冻结 AD 的原生开环规划输出、投影方向及 0.5 s 步长运动学诊断 | 输入敏感性门已通过；CF-R-PLAN-001 中三档冲突在 10 个充分 warm-up 时刻产生严格有序的 3 s 纵向规划响应，无反转或模式切换 | 响应幅度符合现实、显式关键目标身份、校准风险概率、制动/转向控制、闭环安全或 HUGSIM 整体可信 | 先资格化 plan→HUGSIM loop 的轴、时间与控制接口；之后做有界闭环结果实验，并以匹配真实结果或独立行为范围升级外部效度 |
| Planning / response audit | 原生候选模式分解、模式分数间隔和路线进展 | 固定 SparseDrive 的六个原生候选、分数、选中轨迹与声明 actor 几何；不使用 HUGSIM scorer | 把选中轨迹的强弱条件差异拆成固定模式响应和模式选择贡献，并与本地重复误差比较 | 同窗口实验中，frame 48 被定位为模式选择掩盖固定模式减速；frame 54 被定位为全部固定模式的小幅反转；该分解可作为工程诊断 | 模式语义正确、响应符合现实、风险概率、物理 TTC、碰撞或 AD 安全；当前 actor 静态净距已因缺少未来时间对齐而拒绝作为动态风险指标 | 匹配真实 AD 响应或独立行为范围；完整未来 actor 状态、ego footprint 和有效时域；必要时异构接收方 |
| Planning / complete-future conflict | 共同参考路径及 SparseDrive 自身规划下的时间对齐未来 footprint 净距、最近时刻和零净距事件 | 计划、ego/actor box 来自现有 HUGSIM–SparseDrive 运行；时间匹配、定向 footprint 和裁决由第三方脚本重算，不使用 HUGSIM scorer；状态 box 仍依赖 HUGSIM | 把“干预造成的冲突刺激”与“目标 AD 增加的净距响应”分开，并严格排除未来状态不完整的尾窗 | CF-R 现有四次运行中仅前 `4/9` 个计划具有完整 3 s 未来；共同路径的弱–强净距差为 `4.750 m`；SparseDrive 在强/弱条件分别增加约 `1.351/0.696 m` 净距，响应差 `0.655 m` 超过 `0.000004312 m` reset 范围，且模式均为 `3` | 物理 TTC、校准风险、状态 box 现实真值、响应幅度合理性、碰撞概率、AD 或 HUGSIM 整体可信 | 独立三维/运动参考、匹配现实响应或外部行为范围、更多冲突类型与场景；仅在结论依赖接收方时增加异构 AD |
| Task / external following boundary | UN R157 M1/N1 同车道最小跟车距离与计划 longitudinal bumper gap 的余量 | 边界来自外部法规；速度、footprint 和未来状态来自 HUGSIM/SparseDrive，关系由第三方脚本重算；该法规不是通用风险真值 | 检查现有 CF-R 是否覆盖一个可辩护的外部任务边界，而不从仿真结果反推阈值 | `120/120` 样本适用且全部高于边界；最小余量 `6.789 m`、最小 gap/boundary `4.39×`；因此当前实验只验证相对响应方向，不验证临界安全行为 | 现实安全、完整 UN R157 合规、物理 TTC、边界唯一正确、响应幅度真实或 HUGSIM 整体可信 | 以同一外部比较器设计 above/near/below 三档边界覆盖；之后仍需独立状态和目标 AD/ODD 依据 |
| Task / external-boundary response | 外部边界两侧的共同路径刺激、同模式纵向规划响应、完整净距响应和可执行闭环 | `4/2/1 m` 刺激由冻结几何反推；SparseDrive 原生计划独立读取；精确 actor future 来自完整状态运行；不使用 HUGSIM scorer | 区分“接收方方向响应”“完整净距指标有效”和“控制接口可执行”三件事 | 刺激覆盖与 mode-3 纵向端点顺序 `1.369 < 1.658 < 3.253 m` accepted；near/below 近停导致末点 heading/same-lane 门失效，完整净距 rejected；三档均因 `0.356–0.400 rad/s` 超过 `0.261799 rad/s` 接口上限而未完成闭环 | 近停 footprint heading、饱和后的物理响应、最终风险后果、真实响应幅度、安全或 HUGSIM 整体可信 | 先资格化近零速 heading 与目标车辆执行器/跟踪器合同；保持三档刺激不变再双重复 |
| Control / actuation contract | `strict_audit` 原始可执行性、`bounded_projection` 动作投影负担、最终 progress/speed 与重复范围 | 动作名称、语义和 box 边界由 HUGSIM source/config 确认；raw/applied 由独立合同层逐步记录；状态演化仍来自 HUGSIM | 区分原始控制不可执行、显式投影后的执行能力，以及受约束闭环内部响应方向 | strict 在第 9 次尝试保留 `0.4 > 0.261799 rad/s` 并在应用前停止；六个 bounded run 均完成 18/18 steps 且 applied 全部在 box 内，合同机制 `accepted` | bounded 不是物理执行器；progress/speed 的 `below <= near <= above` 在两次 reset 均因 near<below 而 `rejected`；near 末速 `-0.270/-0.337 m/s`，不能形成真实车辆、近停净距或安全结论 | 先资格化 forward/reverse、零速和 near-stop heading 的车辆状态转换合同；随后才判断是否对相同 4/2/1 m 条件作 corrective rerun |
| Control / outcome | acceleration、steer rate、车辆状态、碰撞和任务结果 | SparseDrive live 六相机接收方→受审计 FIFO→修正坐标的 HUGSIM iLQR；直接状态几何独立重算 | 新观测能否产生新计划；强弱冲突能否形成大于同条件重复漂移的车辆结果 | CF-R closed-loop 001 中强/弱条件各双重复；最终前进距离、车速和 oriented-box 净距均保持预注册方向，最小条件效应分别为条件内漂移的约 `34.6× / 3.5× / 54.3×` | 响应幅度符合现实、物理 TTC、真实安全或一般场景有效性；精确数值闭环复现仍被拒绝 | 以匹配真实结果或独立行为范围资格化响应幅度；若只换接收方，只能审计模型依赖性而不能替代现实锚点 |
...
| Source dynamic mask ↔ native-render support | source RGB 预处理得到的动态 mask 与 factual-minus-static 原生动态像素贡献 | mask 不读取 HUGSIM render/semantic/depth，但共享 source geometry、语义预处理和重建输入；属于 partial reference | 检查原生动态对象的 camera membership、像素支持和跨相机过渡，不作传感器等效 | 两个 reader-declared test 时间戳的 `12/12` camera membership 一致；3 个支持视图的 exact-mask energy fraction 为 `0.750–0.805` | actor-free 静态参考、独立人工/真实标注、更多对象/场景和下游 receiver consequence |
| Natural-actor real/factual/static receiver bridge | 同一 SparseDrive 在真实 RGB、原生动态 factual 和省略动态路径的 static render 下的规划差异 | real RGB 为 source-side 观测；两种 render 共享 HUGSIM 与静态残影，SparseDrive 是目标 AD 而非真值 | 检查“像素支持到达接收方”及其是否让任务输出靠近真实响应 | frame 44 的 factual–static ADE `0.0947 m` 远超 repeat；但 real–factual ADE `0.0816 m` 大于 real–static `0.0624 m`；后续又发现 static/factual 均保留同一 rank-2 pedestrian 输出，故 static 被拒绝为 actor-absence control | clean same-pose background、更多 source 对象与场景、接收方依赖性和外部任务边界 |
