...
| Control / outcome | acceleration、steer rate、车辆状态、碰撞和任务结果 | SparseDrive live 六相机接收方→受审计 FIFO→修正坐标的 HUGSIM iLQR | 新观测能否产生新计划、动作方向、车辆状态演化和重复运行敏感性 | 无 actor 的 2 s live gate 已连续产生 4 个新计划；接口、动作边界和反馈能力通过，两个复位保持同模式与动作符号 | 精确闭环数值复现、冲突下避碰正确、动作幅度符合现实或真实车辆安全 | 下一 CF-R 每个冲突条件至少双重复；要求条件间效应大于条件内闭环漂移，再以匹配真实结果或独立行为范围升级 |
